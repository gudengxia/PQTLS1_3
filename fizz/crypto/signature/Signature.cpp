...
#include <fizz/crypto/signature/oqssig.h> //fzhang
...
/*-------------------------------fzhang start-------------------------------*/
std::unique_ptr<folly::IOBuf>pqSign( //fzhang
    folly::ByteRange data,
    const folly::ssl::EvpPkeyUniquePtr& pkey) {
 /*auto out = folly::IOBuf::create(EVP_PKEY_size(pkey.get())); 
  unsigned int bytesWritten = 0;
  if ((pkey_oqs_digestsign(pkey.get(), (unsigned char*) out->writableData(), (size_t*)&bytesWritten, (unsigned char*)data.data(), data.size())) != 1) {
    throw std::runtime_error("Failed to sign");
    }
  out->append(bytesWritten);
  unsigned char* p = out->writableData();*/
  std::unique_ptr<unsigned char[]> sig(new unsigned char[EVP_PKEY_size(pkey.get())]);
  size_t sig_len;
  if ((pkey_oqs_digestsign(pkey.get(), sig.get(), &sig_len, data.data(), data.size())) != 1) {
    throw std::runtime_error("Failed to sign");
    }

  return IOBuf::copyBuffer(sig.get(), sig_len);
}

void pqVerify( //fzhang
    folly::ByteRange data,
    folly::ByteRange signature,
    const folly::ssl::EvpPkeyUniquePtr& pkey) {
  //cout<<"signature length'"<<signature.size()<<endl;
  if ((pkey_oqs_digestverify(pkey.get(), (uint8_t*)signature.data(), signature.size(), (uint8_t* )data.data(), data.size())) != 1) {
    throw std::runtime_error("Signature verification failed");
  }
}
/*-------------------------------fzhang end-------------------------------*/
