...
int fizzClientCommand(const std::vector<std::string>& args) {
  ...
  /*----------------------------------fzhang start----------------------------------*/
  std::vector<NamedGroup> supportedgroups {
      NamedGroup::p256_kyber512, NamedGroup::p384_kyber768, NamedGroup::p521_kyber1024,
      NamedGroup::x25519, NamedGroup::secp256r1, NamedGroup::secp384r1, NamedGroup::secp521r1};
  std::vector<SignatureScheme> supportedSigSchemes = {
      SignatureScheme::dilithium2,  SignatureScheme::p256_dilithium2,
      SignatureScheme::dilithium3,  SignatureScheme::p256_dilithium3,
      SignatureScheme::dilithium4,  SignatureScheme::p384_dilithium4,
      SignatureScheme::mulan, SignatureScheme::p256_mulan,
      SignatureScheme::aigis, SignatureScheme::p256_aigis,
      SignatureScheme::falcon512, SignatureScheme::p256_falcon512,
      SignatureScheme::falcon1024, SignatureScheme::p521_falcon1024,
  ...
  FizzArgHandlerMap handlers = {
    ...
    {"-sig", {true, [&supportedSigSchemes](const std::string& arg) {
	      supportedSigSchemes.clear();
        supportedSigSchemes = splitParse<SignatureScheme>(arg);
    }}},//fzhang
    {"-kex", {true, [&supportedgroups](const std::string& arg) {
	      supportedgroups.clear();
        supportedgroups = splitParse<NamedGroup>(arg);
    }}}//fzhang
  };
  // clang-format on
  ...
  clientContext->setSupportedGroups(supportedgroups); //fzhang
  clientContext->setDefaultShares(supportedgroups); //fzhang
  clientContext->setSupportedSigSchemes(supportedSigSchemes); //fzhang
