...
EventHandler<ClientTypes, StateEnum::ExpectingServerHello, Event::ServerHello>::
  ...
  if (exchange) {
    ...
    Buf serverShare;
    const KeyExchange* kex; //fzhang: note const
    std::tie(group, serverShare, kex) = std::move(*exchange);
    KeyExchange *p = const_cast<KeyExchange*>(kex); //fzhang
    //auto sharedSecret = kex->generateSharedSecret(serverShare->coalesce());
    auto sharedSecret = p->generateSharedSecret(serverShare->coalesce());
    //fzhang change const kex to non-const p
